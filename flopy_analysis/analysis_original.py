...
for subdir, dirs, files in os.walk(path):
    if os.path.exists(subdir + "/" + inDir + "/dem_vb.tif"):  # and float(os.path.relpath(subdir,path)) < 160102040505:
        ...
        # mf1 = flopy.modflow.Modflow(modelname1, exe_name='C:/WRDAPP/64/mf2k5_h5_parallel_64', version='mfnwt')
        # mf2 = flopy.modflow.Modflow(modelname2, exe_name='C:/WRDAPP/64/mf2k5_h5_parallel_64', version='mfnwt')
        # mf3 = flopy.modflow.Modflow(modelname3, exe_name='C:/WRDAPP/64/mf2k5_h5_parallel_64', version='mfnwt')
        # mf4 = flopy.modflow.Modflow(modelname4, exe_name='C:/WRDAPP/64/mf2k5_h5_parallel_64', version='mfnwt')

        # mf1 = flopy.modflow.Modflow(modelname1, exe_name='C:/WRDAPP/MODFLOW-NWT_1.1.2/bin/MODFLOW-NWT_64',
        #                             version='mfnwt')
        # mf2 = flopy.modflow.Modflow(modelname2, exe_name='C:/WRDAPP/MODFLOW-NWT_1.1.2/bin/MODFLOW-NWT_64',
        #                             version='mfnwt')
        # mf3 = flopy.modflow.Modflow(modelname3, exe_name='C:/WRDAPP/MODFLOW-NWT_1.1.2/bin/MODFLOW-NWT_64',
        #                             version='mfnwt')
        # mf4 = flopy.modflow.Modflow(modelname4, exe_name='C:/WRDAPP/MODFLOW-NWT_1.1.2/bin/MODFLOW-NWT_64',
        #                             version='mfnwt')
        ...
        # mf1 = flopy.modflow.ModflowNwt(mf1)
        # mf2 = flopy.modflow.ModflowNwt(mf2)
        # mf3 = flopy.modflow.ModflowNwt(mf3)
        # mf4 = flopy.modflow.ModflowNwt(mf4)
        print "nwt package set up"

        demPath1 = inDir + '/dem_vb.tif'
        ...
        bas3 = flopy.modflow.ModflowBas(mf3, ibound=ibound3)
        bas4 = flopy.modflow.ModflowBas(mf4, ibound=ibound4)
        print "bas package set up"

        #Open hydraulic conductivity and field capacity rasters and get data
        ksatDs = gdal.Open(inDir + "/ksat_vb_ms.tif")
        kvDs = gdal.Open(inDir + "/kv_vb_ms.tif")
        ...
        lpf1 = flopy.modflow.ModflowLpf(mf1, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))
        lpf2 = flopy.modflow.ModflowLpf(mf2, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))
        lpf3 = flopy.modflow.ModflowLpf(mf3, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))
        lpf4 = flopy.modflow.ModflowLpf(mf4, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))

        # flopy.modflow.ModflowUpw(mf1, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))
        # flopy.modflow.ModflowUpw(mf2, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))
        # flopy.modflow.ModflowUpw(mf3, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))
        # flopy.modflow.ModflowUpw(mf4, hk=np.nanmean(ksatData), vka=np.nanmean(kvData))
